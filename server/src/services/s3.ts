...
export const uploadSyndicateImage = async (data: fileUpload.FileArray, syndicateId: string) => {
  const s3 = new S3({ apiVersion: '2006-03-01' });
...
console.log("DATA", data)
  ...
  for (const fileKey in data) {
    const file = data[fileKey] as any;
    const params = {
      Bucket: `lottosyndicatebucket/syndicates/${syndicateId}`,
      Key: file.name,
      Body: file.data,
    };
    const upload = s3.upload(params).promise();
...
const S3Service = { uploadImage, uploadSyndicateImage };
